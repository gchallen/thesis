...
\begin{figure}[t]
\label{lance-sec-eval-figlinear}
\begin{center}
\includegraphics[width=1.0\hsize]{./6-lance/figs/gwa/linear/BOTH.pdf}
\end{center}
\caption{\textbf{Per-node distribution of ADU value and energy usage for the
linear simulation experiment.} 
The top graph shows the amount of data value downloaded from each node, while
the bottom graph breaks down the amount of energy used by each node into the
downloading, routing and overhearing components. Node~1 is closest to the
base station.}
\end{figure}

\begin{figure}[t]
\label{lance-sec-eval-figerror}
\begin{center}
\includegraphics[width=1.0\hsize]{./6-lance/figs/gwa/error/ERROR.pdf}
\end{center}
\caption{\textbf{Effect of cost vector error on optimality.} 
The optimization process is guided by the cost vectors, but predicting the
energy cost of operations before they are performed can be difficult.  Here
we show the impact of introducing a degree of error into the cost vectors
used by the optimizer.  As can be seen, we can tolerate a relatively high
degree of error, as long as the shape of the cost vector does not change.}
\end{figure}
...
Figure~\ref{lance-sec-eval-figlinear} shows simulation results using the
...
\begin{center}
\includegraphics[width=1.0\hsize]{./6-lance/figs/gwa/policy/FILL.pdf}
\end{center}
...
%\begin{figure}[t]
%\begin{center}
%\includegraphics[width=1.0\hsize]{./6-lance/figs/gwa/marm/CDF.pdf}
%\end{center}
...
%\begin{figure}[t]
%\begin{center}
%\includegraphics[width=1.0\hsize]{./6-lance/figs/gwa/motelab/TOPOLOGY.pdf}
%\end{center}
