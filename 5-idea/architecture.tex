...
IDEA is a sensor network service allowing clients to incorporate the
\textit{distributed energy impact} of making local state decisions. By
allowing clients to estimate the \textit{global} energy impact of each state,
IDEA helps them make better \textit{local} decisions. IDEA quantifies the
...
Formally, we can define the problem that IDEA is addressing as follows. At a
...
The interface between client components and IDEA is intended to make it easy
